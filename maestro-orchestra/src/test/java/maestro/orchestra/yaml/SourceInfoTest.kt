...
private fun SourceInfo.assertConsistent() {
    ...
    assertThat(startOffset).isEqualTo(offsetFor(source, startLine, startColumn))
    assertThat(endOffset).isEqualTo(offsetFor(source, endLine, endColumn))
}

private fun offsetFor(source: String, line: Int, column: Int): Int {
    var offset = 0
    var currentLine = 1
    ...
    return (offset + (column - 1)).coerceAtMost(source.length)
}
